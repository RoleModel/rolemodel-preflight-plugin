...
const sortFindings = (
  ...
  const rank = { critical: 0, info: 2, warning: 1 } as const;
  return [...findings].toSorted(
    (first, second) => rank[first.severity] - rank[second.severity]
  );
};

const buildCanvasTargets = (
    ...
    try {
      ...
      if (runtimeErrorReader) {
        for (
          ...
          // Batches run one at a time on purpose — each batch itself is
          // fully parallel (Promise.all below), but firing every batch at
          // once would blow past the intended concurrency cap.
          // oxlint-disable-next-line eslint/no-await-in-loop
      ...
      const runtimeFindings = await Promise.all(
        [...runtimeGroups.entries()].map(async ([key, rows], groupIndex) => {
          ...
          const missingLocalModule =
            /#framer\/local\/codeFile|Unable to resolve specifier/iu.test(
              message
            );
          return {
      ...
      try {
        ...
        const auditedPath = new URL(normalizedUrl).pathname.replace(/\/$/u, "");
        const auditedPage = webPages.find(
          (page) => (page.path ?? "").replace(/\/$/u, "") === auditedPath
        );
      ...
      <section className="panel">
        ...
        {coverage ? (
          <div className="performance-coverage">
            ...
            {coverage.runtimeScanAvailable && coverage.runtimeErrors === 0 ? (
              <p className="panel-muted">
            ...
            {coverage.runtimeScanAvailable ? null : (
              <p className="panel-muted">
